...
function brand_setup() {
	 ...
	 * @link https://developer.wordpress.org/themes/functionality/featured-images-post-thumbnails/
	 */
	 add_image_size( 'post-full', 2000, 1125, true );
...
function brand_get_defaults()
{
    ...
    'page_title_width'                 => 'fullwidth',
    'page_title_alignment'             => 'left',
    'site_title_mobile_spacing_top'    => '0',
    'site_title_mobile_spacing_bottom' => '0',
		'body_bg_color'                    => '#eeeeee',
		'body_text_color'                  => '#777777',
		'link_color'                       => '#222222',
		'link_hover_color'                 => '#1ebaf3',
		'sidebar_layout'                   => 'no',
		'content_layout'                   => 'separated_blocks',
	...
	for ($i=1; $i <= $brand_defaults['slides_number']; $i++) {
		...
		$slides_defaults['brand_slide_button_url' . $i] = 'Button url';
...
function arphabet_widgets_init() {
	...
	register_sidebar( array(
		'name'          => 'Landing Page before content',
		'id'            => 'landing_before_content_1',
		'before_widget' => '<div>',
		'after_widget'  => '</div>',
...
function custom_style() {
	wp_register_style(
		'brand-custom-style',
		get_template_directory_uri() . '/style-custom.css',
		array(),
		BOOTSTRAP_VER,
		'all'
	);
	wp_enqueue_style('brand-custom-style');
}
...
/**
 * Adds custom classes to the body according to content layout setting.
 * @since 1.0
 */
add_filter( 'body_class', 'brand_content_layout_class');
function brand_content_layout_class( $classes='' )
{
	...
	$content_layout = $brand_settings['content_layout'];

	if ( $content_layout === 'one_block' ) :
		$classes[] = 'one-block';
  elseif ( $content_layout === 'separated_blocks' ) :
    $classes[] = 'separated-blocks';
	endif;
...
/**
 * Theme Updater for automatic updates
 * @since 1.5.1
 */
