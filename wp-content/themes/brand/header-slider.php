					...
					<div class="swiper-slide">
						<div id="<?php echo 'brand-slide-' . $i ?>" class="inner-slide" data-text-color="<?php echo esc_attr( $brand_settings['brand_slide_text_color' . $i] ); ?>" style="">
							<div class="slide-content container">
								...
								<h2> <?php echo esc_html( $brand_settings['brand_slide_subtitle' . $i] ) ?> </h2>
								<a href="<?php echo esc_url( $brand_settings['brand_slide_button_url' . $i] ) ?>" class="button"><?php echo esc_html( $brand_settings['brand_slide_button_text' . $i] ) ?> </a>
							</div>
						</div>
					</div> <?php
				}
...
do_action('brand_after_slider');
 ...
 if( ! brand_is_hidden( 'headline' ) && ! brand_use_sections() && !is_front_page() ) {
	 brand_title();
 } ?>
